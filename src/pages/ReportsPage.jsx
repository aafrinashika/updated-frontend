  ...
  return (
    <Sidebar>
      <div className="page-header">
        ...
        <p>View, generate, and download comprehensive email security reports.</p>
      </div>
      ...
      <div className="stat-grid">
        ...
        <div className="stat-card animate-in" style={{animationDelay:'0.1s'}}>
          ...
          <div className="stat-info"><h3>165</h3><p>Phishing Emails</p></div>
        </div>
        <div className="stat-card animate-in" style={{animationDelay:'0.15s'}}>
          ...
          <div className="stat-info"><h3>96%</h3><p>Avg. Accuracy</p></div>
        </div>
      </div>
            ...
            <tbody>
              {monthlyData.map((row, i) => (
                <tr key={i}>
                  ...
                  <td style={{color:'var(--success)', fontWeight:600}}>{row.safe.toLocaleString()}</td>
                  <td style={{color:'var(--danger)', fontWeight:600}}>{row.phishing}</td>
                  <td>
                    <div className="mini-bar-wrap">
                      <div className="mini-bar success-bar" style={{width: `${(row.safe/row.total*100).toFixed(0)}%`}}></div>
                      <span>{(row.safe/row.total*100).toFixed(1)}%</span>
                    </div>
                  </td>
                  <td>
                    <span className="badge badge-safe">{row.accuracy}%</span>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="reports-bottom animate-in" style={{animationDelay:'0.15s'}}>
        <div className="card">
          ...
          <div className="card-body export-buttons">
            ...
            <button className="btn export-btn export-csv">
              <i className="fas fa-file-csv"></i> Export CSV
            </button>
            <button className="btn export-btn export-print">
